// A separate, allowlisted Android payload. Never copy the repository into the APK.
...
async function bundleImage(source) {
  ...
  if (/^http:\/\//i.test(source)) throw new Error(`Android requires HTTPS for external images: ${source}`);
...
await build({ stdin: { contents: legacy + '\n' + entry, resolveDir: appRoot, sourcefile: 'mobile-entry.js' },
  bundle: true, format: 'iife', target: 'chrome89', minify: true,
  outfile: path.join(output, 'assets/app.js'), legalComments: 'eof' });
console.log(`Android web bundle built: ${media.size} optimized images; no workbook, ExcelJS, admin or non-home sections.`);
