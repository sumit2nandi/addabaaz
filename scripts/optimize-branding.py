...
source = Path('backend/media/images/addabaaz-logo.png')
with Image.open(source) as image:
    logo = image.convert('RGB')
    logo.thumbnail((480, 480), Image.Resampling.LANCZOS)
    logo.save('backend/media/images/addabaaz-logo-small.webp', 'WEBP', quality=85, method=6)
    icon = image.convert('RGBA')
    icon.thumbnail((180, 180), Image.Resampling.LANCZOS)
    icon.save('backend/media/images/addabaaz-icon.png', optimize=True)
for name in ['addabaaz-logo-small.webp', 'addabaaz-icon.png']:
    path = Path('backend/media/images') / name
